...
__credits__ = "Pls cite and refer to when using the code: Kronberg R.M.," "Beyond Bloom"


# Set page layout to wide
st.set_page_config(
    page_title="Network Calculation", page_icon="img/otter_logo.jpeg", layout="wide"
)
...
with head000:
    try:
        st.image(
            "img/awi-logo.png",
            caption="The development of this project is support by the Alfred Wegener Institute.",
...
with col1:
    ...
    with my_expander1:
        ...
        # Input fields
        HELLENIGER_NORM = st.checkbox(
            "Hellinger Norm", value=False, help="Use Hellinger Normalisation"
        )
        CON_SYM = st.checkbox(
            "Connectivity Symmetry", value=True, help="Remove directed egdes"
        )
        CON_METHOD = st.selectbox(
            "Connectivity Method",
            ["Pearson_FFT", "Other"],
            help="Select a co-occurrence measurement",
        )
        FFT_COEFFS = st.number_input(
            "FFT Coefficients",
            value=14,
            min_value=1,
            help="Select the number of FFT coefficients",
        )
        CON_TR = st.slider(
...
with col2:
    ...
    with my_expander2:
        ...
        CCMN_SYM = st.checkbox(
            "CCMN Symmetry", value=False, help="False if methods is non symmetric"
        )
        CCMN_METHOD = st.selectbox(
            "CCMN Method", ["NMI", "Other"], help="Select the CCMN Method"
        )
        CCMN_TR = st.slider(
            "CCMN Threshold",
            min_value=0.0,
            ...
            value=0.00,
            step=0.01,
            help="Select the cut-off value for the edge weighs, edge below will be removed.",
        )
        CCMN_ALPHA = st.radio(
            "CCMN Alpha",
            ["yes", "no"],
            help="Select the cut-off of the p_value. Not implemented for CCM using NMI.",
        )
        LOUVAIN_RES = st.number_input(
            "Louvain Resolution",
            value=1,
            min_value=1,
...
with col3:
    ...
    with my_expander3:
        ...
        CALC_CENTRALITIES = st.selectbox(
            "Centralities",
            [False, True],
            help="If checked, theCentralities will be calculated.",
        )
        ...
        NUM_SAMPLES = st.number_input(
            ...
            value=100,
            min_value=10,
            help="Number of samples to calculate the permutation significance",
        )
...
RANDOM_PVAL_CCMN_PATH = f"{PREFIX}/{RUN_ID}_Hellinger_{HELLENIGER_NORM}_{FFT_COEFFS}_Samples_{NUM_SAMPLES}_Permus_{NUM_PERMUTATIONS}_RANDOM_Network.csv"

log_text = {
    "CON_NETWORK_PATH": CON_NETWORK_PATH,
...
with col00:
    ...
    if st.button(
        "Check input data", help="Check if you input data matches our criteria"
    ):
        try:
...
with col8:
    ...
    if st.button(
        "Create Permutation Pruning Network", help="Create the permutation network"
    ):
